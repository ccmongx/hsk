...
insert into tbl_Dienthoai
values('DT4',1,'IPhone14',52000000,20)

insert into tbl_Dienthoai
values('DT5',1,'SamSung Ultra22',29000000,50)

insert into tbl_Dienthoai
values('DT6',1,'Oppo F9',9000000,50)
...
exec ThemHangDT @iMahangDT = 6, @sTenhangDT='Dien thoai moi'
select * from tbl_HangDT
...
alter proc kiemtra (@sMaDT varchar(10), @iMaHangDT int, @sTenDT varchar(50), @iSoLuong int, @iGiaBan int)
	as
	select * from tbl_Dienthoai
	where sMaDT=@sMaDT and iMaHangDT=@iMaHangDT and @sTenDT=sTenDT and @iSoLuong=iSoLuong and @iGiaBan=iGiaBan


create proc kiemtrakh (@sMaKH varchar(10), @sTenkhachhang varchar(50), @dNgaysinh date, @sDiachi varchar(50), @sGioitinh varchar(10), @sSDT varchar(10) )
	as
	select * from tbl_khachhang
	where sMaKH=@sMaKH and sTenkhachhang=@sTenkhachhang and @dNgaysinh=dNgaysinh and @sDiachi=sDiachi and @sGioitinh=sGioitinh and @sSDT=sSDT



 insert into tbl_khachhang
values('KH1','MAI CHI','2003-12-25','HN','Nu','0123')
Select * from tbl_khachhang